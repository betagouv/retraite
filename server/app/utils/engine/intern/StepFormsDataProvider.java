...
public class StepFormsDataProvider {
	...
	public List<String> getListMois() {
		return asList("Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre",
				"Décembre");
	}

	public List<ValueAndText> getListMoisAvecPremier() {
		final List<ValueAndText> listMoisAvecPremier = new ArrayList<>();
		final List<String> listMoisSansPremier = getListMois();
		for (int i = 0; i < listMoisSansPremier.size(); i++) {
			listMoisAvecPremier.add(new ValueAndText(String.valueOf(i + 1), "1er " + listMoisSansPremier.get(i)));
		}
		return listMoisAvecPremier;
	}
